...
node3=$(kubectl --kubeconfig ${kubeconfig} get nodes -l node-role.kubernetes.io/worker | awk '{print $1}' | head -n 2)
...
comment "There is a dedicated node for ACM add-ons on local-cluster"
pe "kubectl describe nodes ${node1} | grep Taints"
pe "kubectl get nodes ${node1} --show-labels"

comment "There is a dedicated node for ACM add-ons on cluster1"
...
comment "Case1: Add a global default addondeploymentconfig for add-ons"
comment "Create a AddOnDeploymentConfig in the open-cluster-management-hub namespce on the hub cluster"
pe "cat configs/global_config.yaml"
pe "kubectl -n open-cluster-management-hub apply -f configs/global_config.yaml"

comment "Patch the ClusterManagementAddons for each add-on to specify the default config with global config on the hub cluster"
...
comment "Each add-on will reference the global config on all managed clusters"
...
comment "Each add-on should run on the dedicated node"
...
comment "Case2: Specify a config for managed-serviceaccount to override its default config on the cluster1"

comment "There is a dedicated node for managed-serviceaccount on cluster1"
